				...
				<main class="flex-1 overflow-x-hidden overflow-y-auto" id="main">
					<div class="container mx-auto px-6 py-6">
						<section class="container font-mono w-full">
							<div class="w-full flex justify-between">
								<h3 class="text-2xl pb-2">All Available Song</h3>
								<div class="mb-6 flex justify-end">
									<a class="inline-flex items-center ml-auto bg-indigo-500 hover:bg-indigo-700 hover:text-gray-200 text-white font-bold py-2 px-4 rounded" href="/song-add">+ New Song</a>
								</div>
							</div>
							<div>
								<table class="w-full">
									<thead>
										<tr class="text-md font-semibold tracking-wide text-left text-gray-900 bg-gray-100 uppercase border-b border-gray-600">
											...
											<th class="px-4 py-3">Instrumentes</th>
											<th class="px-4 py-3">Thumbnail</th>
											<th class="px-4 py-3">Song URL</th>
                  ...
                  @foreach($songs as $song)
										<tr >
											...
											<td class="px-4 py-3 border">
												<span class="font-semibold">{{ $song->price }}$</span>
											</td>
											<td class="px-4 py-3 border">
												<span class="font-semibold">{{ $song->release }}</span>
											</td>
											<td class="px-4 py-3 border">
												<span class="font-semibold">{{ $song->genres }}</span>
											</td>
											<td class="px-4 py-3 border">
												<span class="font-semibold">{{ $song->instrument }}</span>
											</td>								
											<td class="px-4 py-3 border">
												<span class="font-semibold"><img class="admin-thumbnail" src="{{ URL('public/thumbnail/'.$song->thumbnail) }}" alt="he"/></span>
											</td>
											<td class="px-4 py-3 border">
												<span class="font-semibold"><div class="iframe"><iframe style="border-radius:12px" src="https://open.spotify.com/embed/track/{{ $song->song_url }}?utm_source=generator" width="100%" height="152px" frameBorder="0" opacity="70%" allowfullscreen="" allow="autoplay; clipboard-write; encrypted-media; fullscreen; picture-in-picture" loading="lazy"></iframe></div></span>
											</td>	
											<td class="px-4 py-3 text-sm border" width="300">
												<a class="px-2 py-1 font-semibold leading-tight text-yellow-700 bg-yellow-100 active:bg-yellow-200 rounded-sm ml-2" href="{{ route('song.edit',$song->id) }}">Update Song</a>
												<a class="ml-2 px-2 py-1 font-semibold leading-tight text-purple-700 bg-purple-100 rounded-sm mr-4 disabled" href="{{ route('song.delete',$song->id) }}">Delete Song</a>
											</td>
										</tr>
										@endforeach
									</tbody>
